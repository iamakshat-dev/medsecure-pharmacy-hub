...
import { Home, ShoppingCart, Truck, LogOut, Pill, Lock, Menu, X, Sun, Moon } from 'lucide-react';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from '@/components/ui/dialog';
import { Button } from '@/components/ui/button';
import { useTheme } from '@/hooks/useTheme';

const navItems = [
  { path: '/distributor', label: 'Home', icon: Home },
  { path: '/distributor/orders', label: 'Orders', icon: ShoppingCart },
  { path: '/distributor/tracking', label: 'Tracking', icon: Truck },
];
      ...
      <nav className="sticky top-0 z-50 border-b border-border bg-card/80 backdrop-blur-lg">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link to="/distributor" className="flex items-center gap-2">
            ...
            <span className="ml-2 rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-medium text-primary">Distributor</span>
          </Link>
